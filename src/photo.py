...
import requests, random, pathlib, os

# Setup
apis = ["https://api.gumengya.com/Api/DmImgS?format=image", "https://api.gumengya.com/Api/FjImg?format=image", "https://api.yimian.xyz/img", "https://img.paulzzh.tech/touhou/random", "https://www.dmoe.cc/random.php", "https://api.paugram.com/wallpaper/"]
...
def main():
    ...
    if int(time()) % random.randint(1, 10) in [random.randint(1, 10) for each in range(random.randint(1, 8))]:
        ...
        print(f"Since the random condition holds, we will adjust the number of runs to {RUN_TIMES} times")
    else:
        RUN_TIMES = 1
    ...
    if int(time()) % random.randint(1, 10) in [random.randint(1, 10) for each in range(random.randint(1, 10))]:
        print(f"Since the random condition holds, we will archive to {HISTORY_PATH}")
        Archive = True
        HISTORY = [int(pathlib.Path(each.replace("photo", "").replace("-", "")).stem) for each in os.listdir(HISTORY_PATH)]
        HISTORY.sort()
        Archive_PATH = HISTORY_PATH / f"photo-{HISTORY[-1] + 1}.png"
        del HISTORY
    else:
        print(f"Since the random condition doesn't hold, we won't archive to {HISTORY_PATH}")
        Archive = False
    ...
    while CONTINUE:
        ...
        try:
            ...
            print(f"We've got an error{f'(detail: {e})' if e.__str__() else ''}, We're going to Retry")
    ...
    print(f"We have got the response(URL: {response.url}), We're going to write-in")
